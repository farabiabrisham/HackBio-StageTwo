...
library(readr)
f_Table_1 <- read_csv("Desktop/Internships and Workshops/HackBio DataViz/3. Stage Two/hb_stage2/csv files/f-Table 1.csv")
...
cols <- c("Plasma" = "#3E5F8A", "B" = "pink")  # tweak if you want
...
mat <- as.data.frame(g_Table_1)

rownames(mat) <- mat[[1]]
mat <- mat[, -1]                 # drop first column
mat <- as.matrix(mat)

# Make sure it's numeric
mat <- apply(mat, 2, as.numeric)
rownames(mat) <- rownames(g_Table_1)
...
# 4) Remove zero-weight edges
g <- delete_edges(g, E(g)[weight == 0])

# 5) Scale edge width + arrow size by weight
w <- E(g)$weight
E(g)$width <- 1 + (w / max(w)) * 4
E(g)$arrow.size <- 0.3 + (w / max(w)) * 0.8
E(g)$color <- "grey"
...
## Directed cell–cell interaction network (Panel 2g)
